...
_llm_is_disabled_for_process = False
_runtime_config_logged_for_process = False
...
def log_runtime_configuration() -> None:
    """Log the effective OpenAI runtime configuration once per process."""
    global _runtime_config_logged_for_process

    if _runtime_config_logged_for_process:
        return

    logger.info(
        "OpenAI LLM extraction config: enabled=%s endpoint=%s model=%s",
        is_configured(),
        OPENAI_API_URL,
        get_configured_model(),
    )
    _runtime_config_logged_for_process = True
...
def extract_vendor_intelligence(
    ...
    *,
    ...
    global _llm_is_disabled_for_process

    if not is_configured() or _llm_is_disabled_for_process:
        return None
                ...
                "content": [
                    {
                        ...
                        "text": (
                            ...
                            "- pricing must be an array of short strings\n"
    ...
    try:
        ...
        if 400 <= response.status_code < 500 and response.status_code != 429:
            ...
            logger.warning(
                "Disabling LLM extraction for the rest of this process after OpenAI returned %s: %s",
                response.status_code,
                error_body,
            )
            _llm_is_disabled_for_process = True
            return None
...
def _response_body_snippet(response: requests.Response) -> str:
    ...
    return response_text[:MAX_ERROR_BODY_CHARS]
...
def _build_site_text(page_payload: dict[str, object]) -> str:
    ...
    for page_key in ["homepage", "product_page", "pricing_page", "case_studies_page", "about_page", "security_page"]:
        section_text = str(page_payloads.get(page_key, {}).get("text", "")).strip()
        section_url = str(page_payloads.get(page_key, {}).get("website", "")).strip()
        if not section_text:
            continue
